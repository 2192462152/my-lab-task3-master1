<template>
  <div class="container">
    ...
    <el-breadcrumb separator="/" style="margin-bottom: 20px;">
      <el-breadcrumb-item @click="goBack" style="cursor: pointer;">冰箱导航</el-breadcrumb-item>
      <el-breadcrumb-item>{{ roomInfo.device_name || '机房详情' }}</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card style="margin-bottom: 20px;">
      ...
      <el-descriptions :column="3" border>
        <el-descriptions-item label="冰箱名称">{{ roomInfo.device_name }}</el-descriptions-item>
        <el-descriptions-item label="冰箱编号">{{ roomInfo.number }}</el-descriptions-item>
        <el-descriptions-item label="创建时间">{{ formatDateTime(roomInfo.ctime) }}</el-descriptions-item>
        <el-descriptions-item label="描述" :span="3">{{ roomInfo.remarks || '暂无描述' }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
          ...
          <div class="video-container">
            <div v-if="selectedCamera" class="video-wrapper">
              ...
              <div class="video-player">
                ...
                <!-- 修复加载状态显示 -->
                <div v-else class="loading-stream" v-loading="streamLoading" element-loading-text="正在连接摄像头..." element-loading-background="rgba(0, 0, 0, 0.8)">
                  <div style="height: 350px; display: flex; align-items: center; justify-content: center; background-color: #f5f5f5;">
              ...
              <div class="video-controls">
                <el-button type="primary" size="small" @click="captureFrame" :loading="captureLoading">
                  <el-icon><Camera /></el-icon>
                  截图
                </el-button>
                <el-button type="success" size="small" @click="startAIDetection">
          ...
          <div class="electricity-display">
            ...
            <div class="electricity-content">
              <div class="electricity-value">
                <span class="value">{{ electricityData.electricity || '--' }}</span>
                <span class="unit">kWh</span>
              </div>
              <div class="electricity-info">
                <p class="device-info">设备编号: {{ electricityData.d_no || '--' }}</p>
                <p class="update-time">更新时间: {{ formatElectricityTime(electricityData.time) }}</p>
              </div>
            </div>
          </div>
        </el-card>
...
const switchCamera = async (cameraAddress) => {
  ...
  try {
    const response = await axios.post(`http://localhost:3000/api/camera/${cameraAddress}/switch`)
...
const stopAllCameras = async () => {
  try {
    await axios.post('http://localhost:3000/api/cameras/stop-all')
...
const fetchRoomInfo = async () => {
  try {
    const response = await axios.get('http://localhost:3000/api/allDevices')
    const room = response.data.data.find(item => item.number === roomId.value)
    if (room) {
      roomInfo.value = room
    } else {
...
const fetchSensorData = async () => {
  try {
    const response = await axios.get('http://localhost:3000/api/data1', {
...
const captureFrame = async () => {
  ...
  try {
    ...
    const response = await axios.post(`http://localhost:3000/api/camera/${selectedCamera.value}/capture`)
...
const startAIDetection = async () => {
  try {
    ...
    // 2. 调用AI检测接口
    const response = await axios.post('http://localhost:5000/infer', {  // 组委会提供接口
      image: base64Image
    })
    ...
    if (response.data.inference_results) {
      // 3. 保存AI检测结果到数据库
      try {
        await axios.post('http://localhost:3000/api/behaviorData/add', {
          d_no: roomId.value,
        ...
        const personDetected = response.data.inference_results.some(item => item.label === 'person')
        
        if (personDetected) {
          // 向t_error_msg表插入报警数据
          try {
            await axios.post('http://localhost:3000/api/error-messages', {
              d_no: roomId.value,
              e_msg: '人员报警'
            })
            ...
            ElMessage.success(`AI检测完成，检测到 ${response.data.inference_results.length} 个目标`)
          }
        } else {
          ElMessage.success(`AI检测完成，检测到 ${response.data.inference_results.length} 个目标`)
        }
