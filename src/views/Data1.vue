  ...
  <div class="container">
    <el-card>
      ...
      <el-table :data="data" style="width: 100%">
        ...
        <el-table-column v-for="(item, index) in header" :key="item" :prop="item"
          :label="`${item}(${units[index]})`"></el-table-column>
      ...
      <!-- 图表组件 -->
      <time-series-chart :data="chartData" :header="chartHeader" :units="chartUnits"></time-series-chart>
...
const fetchData = async () => {
  try {
    ...
    const response = await axios.get('http://localhost:3000/api/data1', {params})
...
const fetchBehaviorData = async () => {
  try {
    ...
    const response = await axios.get('http://localhost:3000/api/behaviorData1', {params})
...
const viewFile = (row: any) => {
  ...
  // 构建完整的文件URL
  currentFilePath.value = `http://localhost:3000/${filePath}`
...
//   try {
//     const response = await axios.get('http://localhost:3000/api/behaviorData')
