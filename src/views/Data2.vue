      ...
      <div class="header" style="margin-top: 40px;">
        ...
        <div class="filter-container">
          ...
          <el-date-picker
              ...
              value-format="YYYY-MM-DD HH:mm:ss"
          />
          <el-button type="primary" @click="behaviorHandleSearch">搜索</el-button>
        </div>
      </div>

      <!-- AI检测结果列表 -->
      <el-table :data="behaviorData" style="width: 100%" stripe>
        <el-table-column prop="d_no" label="机房编号" width="120"></el-table-column>
        ...
        <el-table-column label="检测详情" min-width="200">
          <template #default="scope">
            <div v-if="scope.row['检测详情']">
              <el-tag
                v-for="(result, index) in parseDetectionResults(scope.row['检测详情'])"
                :key="index"
                type="success"
                size="small"
        ...
        <el-table-column prop="c_time" label="检测时间" width="180"></el-table-column>
...
const fetchData = async () => {
  try {
    ...
    // }
    const response = await axios.get('http://localhost:3000/api/data2', {params})
...
const fetchBehaviorData = async () => {
  try {
    ...
    const response = await axios.get('http://localhost:3000/api/behaviorData', {params})
...
const fetchSceneIds = async () => {
  try {
    const response = await axios.get('http://localhost:3000/api/sceneIds')
...
const showImageDialog = (imagePath, title) => {
  currentImageUrl.value = `http://localhost:3000/${imagePath}`
...
const deleteAIDetectionRecord = async (id) => {
  try {
    ...
    const response = await axios.delete(`http://localhost:3000/api/behaviorData/${id}`)
