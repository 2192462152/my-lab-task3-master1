...
router.get('/data1', async (ctx) => {
    ...
    try {
        ...
        const [dataResult] = await connection.promise().query(`select d_no, ${fields}, c_time, data_type from t_data${whereClause} order by c_time desc limit 1`, params);
...
router.get('/data2', async (ctx) => {
    ...
    try {
        ...
        // 然后查询分页数据
        let sql = `select ${fields}, DATE_FORMAT(c_time, '%Y-%m-%d %H:%i:%s') as c_time, d_no, data_type from t_data ${whereClause} order by DATE_FORMAT(c_time, '%Y-%m-%d %H:%i:%s') desc limit ? offset ?`;
