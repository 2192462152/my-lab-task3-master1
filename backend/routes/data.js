...
router.get('/realtimeChart', async (ctx) => {
    const { deviceId } = ctx.query;
    ...
    try {
        ...
        const [dataResult] = await connection.promise().query(`select d_no, ${fields ? `${fields},` : ``} c_time, data_type from t_data${whereClause} order by c_time desc limit 3`, params);
